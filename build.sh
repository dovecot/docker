...
amd64_CFLAGS="-g -O2 -fno-omit-frame-pointer -mno-omit-leaf-frame-pointer -flto=auto -ffat-lto-objects -fstack-protector-strong -fstack-clash-protection -Wformat -Werror=format-security -fcf-protection=full"
arm64_CFLAGS="-g -O2 -fno-omit-frame-pointer -mno-omit-leaf-frame-pointer -flto=auto -ffat-lto-objects -fstack-protector-strong -fstack-clash-protection -Wformat -Werror=format-security"
...
for PLATFORM in amd64 arm64; do
	for stage in "-build" "-root" "-dev" ""; do
		...
		docker buildx build --platform $PLATFORM \
			...
			--build-arg PIGEONHOLE_REPO_URL=$PIGEONHOLE_REPO_URL --build-arg PIGEONHOLE_BRANCH=$DOVECOT_BRANCH --target production$stage --tag dovecot/dovecot:$VERSION${stage}-$PLATFORM $VERSION
	done
done
